...
class ThreadSemaphoreManager {
 public:
  static size_t Create(size_t count = 0) {
    sems_.push_back(::sem_t());
    ::sem_init(&(sems_.back()), 0, count);
  ...
  static void Down(size_t index=0) {
    ::sem_wait(&(sems_.at(index)));
  }

  static void Up(size_t index=0) {
    ::sem_post(&(sems_.at(index)));
  }
 private:
  static std::vector<::sem_t> sems_;
};
std::vector<::sem_t> ThreadSemaphoreManager::sems_ = std::vector<::sem_t>();
