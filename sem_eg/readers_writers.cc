...
class Reader : public Thread<Reader> {
 public:
  using ::Thread<Reader>::Thread;
...
int main(/* int argc, char* argv[] */) {
  ...
  for (auto& reader : readers)
    ::pthread_create(&reader.thread_id(),
                     ...
                     ::Reader::Execute,  // pointer to function
...
void* Reader::Execute(void *ptr) {
  ...
  while (true) {
    std::string msg = "\tReader " + std::to_string(reader->id())
      + ": waiting";
