...
class DomainSocketServer {
 public:
  explicit DomainSocketServer(const char *socket_path) {
    socket_path_ = std::string(socket_path);  // let std::string manage char *

    sock_addr_ = {};  // init struct (replaces memset)
    sock_addr_.sun_family = AF_UNIX;  // set to Unix domain socket (than an
                                      //   internet domain socket, for example)
    if (socket_path_[0] == '\0') {
      // abstract socket
      strncpy(sock_addr_.sun_path + 1,  // use strncpy to limit copy for
              socket_path + 1,          //   portability
              sizeof(sock_addr_.sun_path) - 2);  // -2 for leading/trailing \0s
    } else {
      // pathname socket
      strncpy(sock_addr_.sun_path, socket_path, sizeof(sock_addr_.sun_path)-1);
    }
  }
 ...
 private:
  std::string socket_path_;  // let string manage char *

  sockaddr_un sock_addr_;  // address of socket  (try = {0, } or just {0, })
};
...
class DomainSocketClient {
 public:
  explicit DomainSocketClient(const char *socket_path) {
    ...
    sock_addr_ = {};  // init struct (replaces memset by 0-ing out data)
    sock_addr_.sun_family = AF_UNIX;  // set to Unix domain socket (rather than
    ...
    if (socket_path_[0] == '\0') {
      ...
      strncpy(sock_addr_.sun_path,
              socket_path,
              sizeof(sock_addr_.sun_path) - 1);  // -1 for trailing \0
    }
  }
 ...
 private:
  std::string socket_path_;  // use string class to manage char *
                             // use std::unique_ptr for any other data
                             //   type
...
const char kSocket_path[] = "\0./socket_cli_srv_domain_socket";
